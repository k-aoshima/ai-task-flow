...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  // タイマーが非アクティブになったら表示状態をリセット
  useEffect(() => {
    const isActive = !!timerData && timerData.taskId === task.id;
    if (!isActive) {
              ...
              <div className="text-center py-2">
                {!isTimerRevealed && !isExpired ? (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 mx-auto px-4 py-2 rounded-full bg-slate-100 dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-slate-500 dark:text-slate-400 font-medium text-sm border border-slate-200 dark:border-slate-600"
                  >
